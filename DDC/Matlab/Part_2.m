...
coeffNBits = 20;        %Coefficient precision.
coeffNBitsFrac = 19;
coeffNBitsInt = coeffNBits - coeffNBitsFrac;
dataNBits = 25;         %Filter input precision.
dataNBitsFrac = 20;
...
%--------------------------------------------------------------------------
%Decimate input data.
filterOutputReal = filter(scaledCoeff, 1, filterInputRealQ);
newOutput = reshape(filterOutputReal,[2 length(filterOutputReal)/2]);
filterOutputReal = newOutput(1,:);

%Decimate input data.
filterOutputImag = filter(scaledCoeff, 1, filterInputImagQ);
newOutput = reshape(filterOutputImag,[2 length(filterOutputImag)/2]);
filterOutputImag = newOutput(1,:);
...
ylabel('Amplitude');
ylim([-2 2]);
xlim([1 length(filterOutputReal)]);
